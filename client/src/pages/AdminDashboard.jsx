...
import { LayoutDashboard, BarChart3, Users, Settings, AlertTriangle, Flag, SlidersHorizontal, X } from 'lucide-react';
...
const AdminDashboard = () => {
    ...
    const [reportsPagination, setReportsPagination] = useState({ page: 1, pages: 1, total: 0 });
    ...
    const fetchReports = async () => {
        try {
            ...
            const pendingCount = response.data.data.reduce((acc, group) => acc + group.pendingCount, 0);
    ...
    const hasActiveFilters = activeTab === 'issues'
        ? (params.category || params.status || params.search || params.state || params.district)
        : (reportsParams.category || reportsParams.issueStatus || reportsParams.search);
                ...
                <div className="p-4 h-[calc(100%-64px)] overflow-y-auto">
                    {activeTab === 'issues' ? (
                        ...
                        <IssueFilters
                            ...
                            onReset={() => {
                                setReportsParams({ limit: 15, sortBy: 'reportCount', minReports: 1 });
                                setMobileFiltersOpen(false);
                            }}
                            hideCounts={true}
                        />
                    ...
                    <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
                        ...
                        <aside className="hidden lg:block w-[280px] flex-shrink-0 order-1 overflow-y-auto scrollbar-hide">
                            <IssueFilters
                                ...
                                onReset={() => setReportsParams({ limit: 15, sortBy: 'reportCount', minReports: 1 })}
                                hideCounts={true}
                            />
                        </aside>
                                ...
                                <div className="flex flex-wrap items-center gap-4 bg-dark-800 border border-dark-700 rounded-xl p-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => setReportsParams((prev) => ({ ...prev, sortBy: e.target.value, page: 1 }))}
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => setReportsParams((prev) => ({ ...prev, minReports: parseInt(e.target.value), page: 1 }))}
                                            className="px-3 py-1.5 text-sm rounded-lg bg-dark-700 border border-dark-600 text-white focus:outline-none focus:ring-2 focus:ring-primary-500"
                                        >
